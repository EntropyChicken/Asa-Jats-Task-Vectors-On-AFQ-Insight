...
try:
    ...
    sys.stdout.flush()
    from utils import select_device, kl_divergence_loss, prep_fa_dataset, train_vae_age_site_staged, prep_fa_dataset_paired, train_vae_age_site_alternating, train_vae_age_site_alternating_improved
...
try:
    ...
    staged_save_directory = "non_variational_fa_tracts_experiment_results"
    ...
    # ================================================================================
    # ALTERNATING TRAINING EXPERIMENT (OPTIONAL TEST)
    # ================================================================================
    print("\n\n" + "="*80)
    print("TESTING ALTERNATING TRAINING APPROACH - FA TRACTS ONLY")
    print("="*80 + "\n")
    sys.stdout.flush()
    ...
    age_predictor_alt = AgePredictorCNN(input_channels=input_channels, 
                                       sequence_length=sequence_length, 
                                       dropout=age_dropout)
    site_predictor_alt = SitePredictorCNN(num_sites=num_sites, 
                                         input_channels=input_channels, 
                                         sequence_length=sequence_length, 
                                         dropout=site_dropout)

    # Define directory for saving alternating models
    alternating_save_directory = "alternating_fa_tracts_experiment_results"
    os.makedirs(alternating_save_directory, exist_ok=True)
    print(f"Alternating experiment results will be saved in: {alternating_save_directory}")
    sys.stdout.flush()

    print("DEBUG: Starting alternating training")
    sys.stdout.flush()
    try:   
        # alternating_results = train_vae_age_site_alternating(
        #     vae_model=vae_alt,
        #     age_predictor=age_predictor_alt,
        ...
        #     grl_alpha_start=0.0,
        #     grl_alpha_end=10.0,
        #     grl_alpha_epochs=400,
        #     save_dir=alternating_save_directory,
        #     val_metric_to_monitor="val_age_mae",
        #     save_predictions_interval=50,
        #     periodic_save_interval=50,
        #     is_variational=False
        # )
        alternating_results = train_vae_age_site_alternating_improved(
            vae_model=vae_alt,
            age_predictor=age_predictor_alt,
            site_predictor=site_predictor_alt,
            train_data=train_loader_raw,
        ...
        print(f"ERROR during alternating training: {str(e)}")
        import traceback
        print(traceback.format_exc()) 
        sys.stdout.flush()
        # Don't exit, just continue to comparison

    # Compare results if both experiments completed
    if 'staged_results' in locals() and staged_results and 'alternating_results' in locals() and alternating_results:
        print("\n" + "="*80)
        print("COMPARING STAGED vs ALTERNATING TRAINING RESULTS")
        print("="*80 + "\n")
        
        # Process and save alternating training metrics first
        if alternating_results and "alternating" in alternating_results:
            alternating_combined_results = alternating_results["alternating"]
            
            # Convert metrics to CPU floats (same keys as staged training)
            keys_to_convert = [
                "train_loss_epoch", "val_loss_epoch", 
                "train_recon_loss_epoch", "val_recon_loss_epoch",
                "train_kl_loss_epoch", "val_kl_loss_epoch", 
                ...
                "train_age_r2_epoch", "val_age_r2_epoch",
                "current_beta_epoch", "current_grl_alpha_epoch",
                "current_lr_epoch", "training_phase_epoch"  # Include the new phase tracking
            ]
            
            alternating_processed_results = process_metrics(alternating_combined_results, keys_to_convert)
            
            # Create DataFrame for alternating metrics
            num_epochs = len(alternating_processed_results.get("train_loss_epoch", []))
            if num_epochs > 0:
                df_data = {"epoch": range(1, num_epochs + 1)}
                for k in keys_to_convert:
                    col_name = k.replace('_epoch', '')
                    metric_data = alternating_processed_results.get(k, [])
                    if len(metric_data) != num_epochs:
                        metric_data = [float('nan')] * num_epochs
                    df_data[col_name] = metric_data
                
                df_epochs = pd.DataFrame(df_data)
                alternating_metrics_file = os.path.join(alternating_save_directory, "alternating_combined_metrics.csv")
                df_epochs.to_csv(alternating_metrics_file, index=False)
                
                # Save summary metrics for alternating training
                best_mae_key = "best_val_age_mae"
                best_mae = alternating_combined_results.get(best_mae_key, float('nan'))
                if isinstance(best_mae, torch.Tensor):
                    best_mae = float(best_mae.cpu().item())
                
                df_summary = pd.DataFrame([{
                    best_mae_key: best_mae,
                    "best_epoch": alternating_combined_results.get("best_epoch", float('nan')),
                    "model_path": alternating_combined_results.get("model_path", "N/A")
                }])
                alternating_summary_file = os.path.join(alternating_save_directory, "alternating_combined_summary.csv")
                df_summary.to_csv(alternating_summary_file, index=False)
                
                print(f"Saved alternating training metrics to {alternating_metrics_file}")
                print(f"Saved alternating training summary to {alternating_summary_file}")
        
            # Process and save alternating VAE Stage 1 results
            if "vae" in alternating_results:
                vae_results = alternating_results["vae"]
                ...
                if vae_epochs > 0:
                    ...
                    vae_df = pd.DataFrame(vae_df_data)
                    vae_metrics_file = os.path.join(alternating_save_directory, "alternating_vae_metrics.csv")
                    vae_df.to_csv(vae_metrics_file, index=False)
                    print(f"Saved alternating VAE training metrics to {vae_metrics_file}")

            # Process and save alternating Age Predictor Stage 1 results
            if "age_predictor" in alternating_results:
                age_results = alternating_results["age_predictor"]
                ...
                if age_epochs > 0:
                    ...
                    age_df = pd.DataFrame(age_df_data)
                    age_metrics_file = os.path.join(alternating_save_directory, "alternating_age_predictor_metrics.csv")
                    age_df.to_csv(age_metrics_file, index=False)
                    print(f"Saved alternating Age Predictor training metrics to {age_metrics_file}")

            # Process and save alternating Site Predictor Stage 1 results
            if "site_predictor" in alternating_results:
                site_results = alternating_results["site_predictor"]
                ...
                if site_epochs > 0:
                    ...
                    site_df = pd.DataFrame(site_df_data)
                    site_metrics_file = os.path.join(alternating_save_directory, "alternating_site_predictor_metrics.csv")
                    site_df.to_csv(site_metrics_file, index=False)
                    print(f"Saved alternating Site Predictor training metrics to {site_metrics_file}")
        
        # Extract best metrics from both approaches
        staged_best_mae = staged_results.get("combined", {}).get("best_val_age_mae", float('inf'))
        alternating_best_mae = alternating_results.get("alternating", {}).get("best_val_age_mae", float('inf'))
        
        print(f"Staged Training Best Val Age MAE: {staged_best_mae:.4f}")
        print(f"Alternating Training Best Val Age MAE: {alternating_best_mae:.4f}")
        
        if alternating_best_mae < staged_best_mae:
            improvement = ((staged_best_mae - alternating_best_mae) / staged_best_mae) * 100
            print(f"🎉 Alternating training improved by {improvement:.2f}%!")
        elif staged_best_mae < alternating_best_mae:
            degradation = ((alternating_best_mae - staged_best_mae) / staged_best_mae) * 100
            print(f"📊 Staged training was better by {degradation:.2f}%")
        else:
        ...
        comparison_df = pd.DataFrame([{
            "approach": "staged",
            "best_val_age_mae": staged_best_mae,
            "best_epoch": staged_results.get("combined", {}).get("best_epoch", -1)
        }, {
            "approach": "alternating", 
            "best_val_age_mae": alternating_best_mae,
            "best_epoch": alternating_results.get("alternating", {}).get("best_epoch", -1)
        }])
        comparison_file = "training_approach_comparison.csv"
